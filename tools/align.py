...
LABEL_SOURCES = {"schemaorg": "cache/schemaorg.pkl", "yago": "cache/yago.pkl", "wikidata": "cache/wikidata.pkl", "gpt": "cache/gpt.pkl",
                 ...
                 "iana": "cache/iana.pkl",          # never matched by label either: a subtype is a bare token (text/calendar, model/step) that collides with unrelated words; aligned by tools/align_iana.py (media-types.tsv)
...
def read_media_types(d):
    """name -> IANA media type (<pack>/align/media-types.tsv: name  media-type  note),
    GENERATED by tools/align_iana.py.  Keyed by NAME, not by synset: the formats it
    aligns are the pack's own concepts and most of them have no WordNet sense at all
    (webp, cbor, opentype).  A hand line in overrides.tsv is read later and wins."""
    out = {}
    p = d / "media-types.tsv"
    if p.exists():
        for row in csv.reader(open(p), delimiter="\t"):
...
def main():
    ...
    iana_map = read_media_types(dirs.align)
    ...
    # A format family the pack asserts only in extra-edges.tsv (image-file-format,
    # text-file-format) is a name with no row here, so it had nowhere to carry its
    # media type and the registry entailed nothing at all (2026-09-17, §14: the first
    # iana view came out empty).  A name the media-type table aligns gets a row.
    for name in iana_map:
        concepts.setdefault(name, None)
    for d in drops:
        concepts.pop(d, None)
    ...
    for name, off in sorted(concepts.items()):
        ...
        for s in LABEL_SOURCES:
            ...
            if s == "cpc":
                ...
                continue
            if s == "iana":
                row[s] = iana_map.get(name, "") if iana_map.get(name, "") in graphs[s].nodes else ""
                continue
