...
LABEL_SOURCES = {"schemaorg": "cache/schemaorg.pkl", "yago": "cache/yago.pkl", "wikidata": "cache/wikidata.pkl", "gpt": "cache/gpt.pkl",
                 ...
                 "iana": "cache/iana.pkl",
                 "isco": "cache/isco.pkl",          # never matched by label: its group titles are phrases; aligned by synset (packs/occupations/align/occupations.tsv, GENERATED by tools/align_isco.py)          # never matched by label either: a subtype is a bare token (text/calendar, model/step) that collides with unrelated words; aligned by tools/align_iana.py (media-types.tsv)
...
def read_isco_map(d):
    """offset -> ISCO-08 code (<pack>/align/occupations.tsv: offset  name  isco  note), GENERATED by
    tools/align_isco.py from the ILO's index of occupational titles (docs/OCCUPATIONS.md). By synset,
    never by label, as CPC; a core concept's code comes through the pack's overrides.tsv instead."""
    out = {}
    p = d / "occupations.tsv"
    if p.exists():
        for row in csv.reader(open(p), delimiter="\t"):
...
def main():
    ...
    isco_map = read_isco_map(dirs.align)
    ...
    for name, off in sorted(concepts.items()):
        ...
        for s in LABEL_SOURCES:
            ...
            if s == "cpc":
                ...
                continue
            if s == "isco":
                row[s] = isco_map.get(off, "") if off and isco_map.get(off, "") in graphs[s].nodes else ""
                continue
            if s == "iana":
