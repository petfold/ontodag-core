...
(every disputed pair in core and in every pack settled; core's 362 unplaced concepts cut to 3 by giving it the
...
files with `sh tools/build.sh`; check that core and every pack merge into one coherent store with `python3 tools/regen_ontodag_core.py $(git rev-parse --short HEAD)`, `python3 tools/regen_ontodag_packs.py $(git rev-parse --short HEAD)` and then `python3 tools/integrate.py` — regenerate the shipped modules FIRST or the run is against a stale core (UPPER.md §8.1); a pack with
