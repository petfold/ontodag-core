...
| geography / Earth / space | `place`, `region`, `land`, `body-of-water`, `landform`, `continent`, `ocean`, `sea`, `river`, `lake`, `mountain`, `island`, `weather`, `climate`, `rock`, `soil`, `map`, `geography`, `earth-science`, `celestial-body`, `planet`, `star` — all in core | physical geography, geodesy and cartography (coordinate systems, datums, projections, great circles, GNSS), Earth structure and plate tectonics, the geologic time scale, landforms and water bodies, meteorology and climate, oceanography; the seven continents, five oceans, poles, tropics and a few unmistakable features as instances; **no countries, cities or borders**. Space is the sibling `space` pack. | WordNet's geography/geology/meteorology/oceanography/geodesy topics and ~70 cones; ~510 Wikidata items resolved by `tools/resolve_labels.py`; hand-asserted hinges (`earth-sphere`, `atmospheric-layer`, `geographic-line`, `geographic-data-format`) |
...
|---|---|---|---|
| physics | 195 | WordNet physics/astronomy topics, the particle cone; 12 new unit heads under `linear-dimension` | particle, concept (physical-law, theory), natural-event (physical-phenomenon), event, place, attribute |
...
| economics | 1126 | Markets, finance, trading, investing, accounting, payments, insurance, contracts, logistics, Bitcoin and the Ethereum ecosystem. **Scope rule (Peter): only the universally accepted parts — no macroeconomics, no government-driven economics (tax, monetary policy, welfare), no Solana or other shitcoins; if in doubt, leave it out.** WordNet: the commerce, payment, cost, income, debt, loan, credit, security, bond, stock, share, insurance, fund, price, fee, contract, merchant, carrier, transport and delivery cones (~150 agriculture/industry/government/gambling/illicit-trade leftovers dropped by hand); **Wikidata**: ~520 roots resolved by exact-label SPARQL (the search API had slowed to 4 s a term) and read against descriptions; **hand-asserted** via `extra-edges.tsv` the ~80 concepts Wikidata has no item for — mempool, hash rate, halving, block reward, Lightning channels, gas, ERC-721, rollups, validators, the Beacon Chain, MEV, account abstraction, and Swarm's postage batches and chunks. | financial-services, commerce, commercial-trade, monetary-exchange, trading, investing, contract, legal-document, payment, payment-amount, purchase-price, income, expense, fee, debt, loan, credit, share-capital, bond, fund, merchant, transportation, transport, possession, information, concept, procedure, act, event, state, organization, company, software, device, container, vehicle, ship, aircraft, building, crime, fraud, agent, person, professional, management, economics, law |
| computing | 1357 | Peter's list (2026-09-03): computing, CS, software engineering, programming languages and concepts, databases incl. functional and Swarm, distributed systems and P2P, web, IR, information theory, networks, Internet, semantic web, KR, knowledge graphs, algorithms outside AI, open source, Linux, Wikipedia/Wikidata, ontologies, data structures, protocols, file formats. WordNet: the computer-science topic plus ~60 cones (software, program, language, code, database, computer, network, hardware, memory device, peripheral, file, data processing, telecommunication, protocol, subroutine, instruction, UI, compiler, OS, email …); everyday lists, telephony, broadcasting, writing systems, recorded music and arithmetic that the cones drag in dropped by hand (~130). **Wikidata**: ~970 items resolved by exact-label SPARQL, the ambiguous ones by type-constrained lookups and the entity-search API; **instances deliberately included** as categories where Peter asked (Linux, Git, Wikipedia, the major languages, protocols, formats) — they hang by P31 in Wikidata, so every one of them, and most abstract concepts whose Wikidata parent is unnamed here, is placed by ruling (~1,150). **Hand-asserted** (`extra-edges.tsv`): the intermediate classes Wikidata lacks or names only as instances' types (image/audio/document/archive/serialization/font file formats, system and server software, networking hardware, the protocol layers, `technical-standard`, `knowledge-organization-system`, `model-of-computation`), and the storage vocabulary this project lives in — `authenticated-data-structure`, `merkle-dag`, `content-identifier`, `append-only-log`, `immutable-database`, `versioned-database`, `functional-database`, `local-first-software`, `decentralized-storage` (shared with economics' `swarm-storage`). | software, computer-program, application, hardware, computer, digital-computer, computer-network, internet, web-site, code, information, interface, chip, document, message, symbol, list, collection, dataset, record, method, procedure, process, activity, practice, skill, property, quantity, concept, rule, law, organization, machine, device, artifact, engineering, mathematics, communication, telecommunication (WordNet) |
| geography | 995 | Peter's list (2026-09-03): geography, Earth, space — only the uncontested, stable and certain; GPS, coordinate systems, map projections, great circles, integrated with the existing space representations. WordNet: the geography, geology, meteorology, oceanography and geodesy topics plus ~70 cones and synsets (geological formation 175, body of water 75, land, atmospheric phenomenon 173, cloud, rock, soil, geological time, season, map, map projection, coordinate system, great circle, latitude, time zone, climatic zone, biome, tide, current, navigational system/instrument, compass point, stratum, the Earth's layers and spheres, sediment, erosion, fossil, surveying instruments); ~130 off-topic senses dropped by hand (anatomy under `layer`, kitchen dregs under `sediment`, pits and pastures, moods of weather, regional and literary words, medical stones under `rock`). **Wikidata**: ~510 items resolved by exact label (`tools/resolve_labels.py`, new) — geodesy (WGS 84, geoid, datums, ECEF, ITRS, ETRS89), the coordinate lines and hemispheres, 25 map projections and their classes, GNSS (GPS, GLONASS, Galileo, BeiDou, DGPS, RTK), time standards (UTC, TAI, GPS time, Unix time, Julian day), Earth's structure and spheres, plate tectonics, the geologic time scale from Hadean to Holocene, landforms, ice, water, oceans, climate classes, cloud genera, storms, the sciences and their instruments, GIS formats. **Instances**: the seven continents, five oceans, poles, tropics and polar circles, prime meridian, Everest, Sahara, Nile, Amazon, Himalayas, Andes, Alps, Great Barrier Reef, Mariana Trench, Dead Sea, Baikal, Greenland, the two ice sheets, the Gulf Stream — physical features with one universal name; **no countries, cities, seas with disputed names, or borders**. Units (nautical mile, knot, degree, arcminute) stay with the registry; the `geo` dimension head is the registry's too — `geohash` here is the concept, filed under `geocode` and `geocoding-system`. | place, region, land, body-of-water, landform, natural-object, physical-object, layer, natural-event, event, weather, climate, rock, material, soil, map, chart, model, document, instrument, device, activity, measure, quantity, angle, line, shape, code, concept, planet, ocean, continent, mountain, river, lake, island, desert, valley, cave, volcano, earthquake, flood, cloud, rain, snow, wind, season, earth-science, geography, geology, geophysics, chemistry, engineering, field-of-study, organization, building, collection, information, dictionary, book, name |
| space | 351 | The sibling of geography for Peter's 'space' (2026-09-03): astronomy and spaceflight, the uncontested part. WordNet: the astronomy cone (the science's branches) and the cones for celestial body, star, planet, natural satellite, minor planet, extraterrestrial object, meteorite, galaxy, nebula, orbit, celestial point (nodes, apsides, equinoctial points, zenith), eclipse, lunar phase, cosmic time, artificial satellite, spacecraft, telescope, plus the spaceflight synsets; the physics pack already holds WordNet's astronomy *topic* nouns, so the two share names (`orbital-node`, `equinoctial-point`, `red-shift`, `astronomical-conjunction`, `orbital-inclination`). **Wikidata**: ~275 items — object types from brown dwarf to supermassive black hole, galaxy types and groupings, the Sun's layers and activity, cosmology (CMB, Big Bang, inflation, Hubble's law, dark matter and dark energy, ΛCDM — the concepts, no numbers), celestial coordinate systems, orbital mechanics (elements, apsides, Lagrange points, Hohmann transfers, the named Earth orbits), eclipses and phases, spaceflight (vehicles, missions, orbits, EVA, agencies), telescopes and catalogues. **Instances**: the Sun, the eight planets, Pluto/Ceres/Eris, the well-known moons, the Solar System, the Milky Way, Andromeda, the Magellanic Clouds, the Local Group, ISS, Hubble, JWST, the Voyagers, Apollo 11, Sputnik 1, the Space Shuttle, Falcon 9, Saturn V, Soyuz. Units (light-year, parsec, AU) stay with the registry. | celestial-body (Wikidata's astronomical object aligned to it), planet, star, sun, moon, satellite (core's is the artificial one), spacecraft, rocket, orbit, constellation, universe, comet, natural-object, natural-event, event, region, layer, point, angle, quantity, concept, substance, radiation, wave, engine, vehicle, instrument, facility, document, code, image, activity, method, organization, company, engineering, astronomy, biology, physics (astrophysics), explosion (big-bang, WordNet) |
...
**Re-run after the second reading (2026-09-03, evening; core v5, seven
packs v2):** union **9,786 categories, 10,850 edges**; sha256 root
`81650b1bf01610530e30afac069d8fc68223e6efbb31023a46a6de102328c2b6`, Swarm/BMT
root `e36871b33e4fa036e658e751bda185e459d56155eb42b0c54e3735bc08c7af28`;
order-independent again (three shuffles, the reverse, a re-merge), every
pack concept reachable, the cross-pack claims resolve, the top level is
core's ten roots plus `dimension`. The first re-run had `major-planet` at
the top: `integrate.py` reads the *shipped* core module from `../ontodag`,
...
modules and the pack files agree byte for byte (0.22.0 re-pins all of them:
core v5 moves every domain pack's adoption root, since that root is core +
pack — the three unchanged packs included). This is the reversible half
...
**Geography (2026-09-03, evening).** Peter's brief was the uncontested and
stable: physical geography, geodesy, cartography, GPS, the Earth's structure,
the geologic time scale, meteorology, oceanography — and no politics. So the
instances are the seven continents, five oceans, poles, tropics and a dozen
features with one universal name (Everest, Sahara, the Nile); no countries,
no cities, no sea whose name two countries dispute. Findings worth keeping:
WordNet's `rock` cone is the *object* sense (a lump), while `igneous rock` and
the rock types hang from the *material* sense (14696793) that core does not
carry — so every rock type arrived unplaced until `rock-material` was added
as a concept; the same split hides behind `layer` (which also drags the skin's
strata in), `sediment` (coffee grounds), `pit` (barbecue pits) and `spring`
(core's name for the water spring is `outflow`, WordNet's first lemma — a poor
core name, flagged). **Core fix:** core's `meteorology` was WordNet's
*prediction* sense ("predicting what the weather will be") and was unplaced;
it is now the science (override), placed under `earth-science` — core v4 grows
by it. Two Wikidata habits to know: P8814 maps several WordNet synsets onto one
item (`bluff`, `precipice` and `cliff` all on Q107679; `feeder` on tributary;
`alp` on the Alps), so `tools/crosspack.py` runs after every build; and its
`geographical feature` class is over-broad (a door, a bridge, a facility, a
summit meeting and `place` itself hang from it) — 58 of 166 Wikidata-only edges
rejected. New tool: `tools/resolve_labels.py`, the exact-label SPARQL resolver
that replaced the throttled search API, kept in the repo this time. One
reading only; ~440 rulings.

**Space (2026-09-03, night).** Two core senses surfaced and were corrected
for v4: core's `star` was WordNet's naked-eye sense ("any celestial body
visible as a point of light"), not the physical star — the space pack, the
physics pack (`stellar-body`) and Wikidata's Q523 all wanted the other one;
and `meteorology` (above). Core keeps `satellite` = the artificial one,
`moon` = Earth's Moon and `sun` = the Sun, so the generic senses are
qualified here: `natural-satellite`, `planetary-moon`, `host-star`,
`planetary-body`. Wikidata's astronomy is good — 16 of 100 single-source
edges rejected, mostly its habit of filing phenomena and media as
`astronomical object` (a gravitational lens, the interstellar medium, an
asterism) and two edges that take a side in a live classification question
(`dwarf planet ⊑ minor planet`; `nova ⊑ binary star`), which the brief
excludes. WordNet's slips: `space probe ⊑ guided missile`, `Orion ⊑ diffuse
nebula`, `black hole ⊑ region`. Cosmology enters as concepts, never as
...
## 10. The everyday goods layer (2026-09-03, night) — core v6

Peter's question: can the products and services traded on loopmarket be
named precisely — supermarket items, clothes, appliances, phones,
furniture, tools, vehicle parts, storage, tickets, treatments? Probed
against the union of core and the ten packs (about 390 terms): the hinges
were there (`clothing`, `furniture`, `appliance`, `vehicle`, `food`,
`service`, `ticket`) and the leaves were not — `appliance` had no children,
`concert`, `flower` and `pen` were leaves; 26 of 56 supermarket words, 48 of
76 home words, 9 of 54 services, 4 of 26 experiences. And it is core-tier:
the everyday-word rule that keeps `toaster` out of a pack keeps `haircut`
and `plumber` out too. So the goods half went into core as v6.

**Source: the Google Product Taxonomy** (`sources/gpt/`, 5,595 nodes, 21
departments, a strict tree, plural labels, a quarter of them union bins), as
a *witness* like SUMO and OpenCyc — coverage and a second source for edge
truth — never imported. `tools/extract_gpt.py` singularises labels (with
an exception list: `glasses`, `jeans`, `scissors` are the kinds), keeps
every "A & B" bin as a node so the path through it survives and splits A and
B out beside it as kinds (`Toasters & Grills` → `toaster`, `grill`); a bin
whose halves share a head (`Fresh & Frozen Vegetables`) *is* that head
(`vegetable`); and same-label nodes merge into one multi-parent node
(gloves under Apparel and under Baseball — the DAG's improvement on the
tree), with a cycle check because a merge can put a head above itself.
`tools/align_gpt.py` aligns each kind label to a WordNet noun synset: a
unique lemma, else the senses under the department's hinge (artifact, the
four food synsets, organism, substance — never person), else the sense under
the GPT parent's own synset; 350 senses were picked by hand
(`align/gpt-picks.tsv`). It writes `align/gpt-synsets.tsv`, which
`align.py` reads like `names.tsv`; a hand list `align/goods-extra.tsv` adds
84 everyday goods GPT has no node for (jeans, sweatshirt, duvet, aspirin,
teddy bear, stroller, wine glass — WordNet has them, GPT's leaf is coarser).
Of 6,347 GPT kinds, 1,058 matched one WordNet noun, 1,164 several senses,
4,125 none: the retail compounds ("guitar stand", "toy boat", 362 "X
accessories") wait for a later layer that can define them.

**What it took to make the witness honest.** GPT's department words match
core words in other senses — `hardware` (computer), `electronics` (the
science), `audio` (a recording), `power` (a person), `cricket` (the
insect), `bakery` (the shop), `drill` (military), `golf`, `tennis`,
`basketball` (the games) — and every child of such a department arrived
with a wrong edge; thirty were cleared (`X gpt -` in `align/overrides.tsv`),
so GPT witnesses nothing through them. The generator's "already in core"
baseline must subtract its own previous output, or the second run finds
nothing new. And a hand-named synset must not reserve its first lemma: the
goods layer's `grinding-machine` (WordNet lemma `grinder`) had silently
renamed core's `grinder` (the sandwich) to `hero-sandwich`, `station` to
`post.location`, `trail` to `lead.cognition`, `shot` to
`scene.communication` — the aligner's `reserved` rule is fixed and the
names are back.

**The reading.** Every WordNet or GPT single-witness edge on the new
concepts — about 1,050 — was read against both glosses in four passes;
about 130 rejected. GPT's failure is filing by department (a canoe under
*sport*, a capacitor under *electronics*, a mailbox under *decor*, a muffin
under *kitchen appliance*); WordNet's are its chains (`wheeled vehicle ⊑
container`, so a scooter is a container; `face powder ⊑ medication`;
`earplug ⊑ occlusion`) and a few slips (`planner ⊑ notebook computer`).
Sense mismatches surfaced in the gloss column and were re-picked (`pants`
...
ceded 29 names to core (thermometer, aspirin, laptop, barometer, scalpel,
cargo) and were regenerated. **Open sense questions**, listed in ontodag's
CORE.md for a v7 decision: `table` (the laid table), `window` (a service
hatch), `case` (display case), `stake` (the bet), `bag` (suitcase), `mask`,
`pool` (puddle), `jelly`, `pepper`, `carrot`/`onion`/`mushroom` (the
plants), `product` (arithmetic), `television` (the system). **Still owed
